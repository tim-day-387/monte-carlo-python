...
class randomGrabAndDuckPlayer(player):
    # Constructor
    def __init__(self, name, randomRate):
        # Inherit
        super().__init__(name)
    ...
    def playCard(self, trick, game):
        ...
        # Decide which method to use, based on threshold
        if getChoice > self.randomRate:
            return self.playCardGrabAndDuck(trick, game)
        else:
            return self.playCardRandom(trick, game)
    
    # Decide which card to play, randomly
    def playCardRandom(self, trick, game):
    ...
    # Decide what card to play, according to grab and duck
    def playCardGrabAndDuck(self, trick, game):
        ...
        if len(trick) != 0:
            ...
            if suit=="T":
                pHand.sort(key = randomGrabAndDuckPlayer.trollKey)
            elif suit=="Z":
                pHand.sort(key = randomGrabAndDuckPlayer.zombieKey)
            elif suit=="F":
                pHand.sort(key = randomGrabAndDuckPlayer.fairyKey)
            else:
                pHand.sort(key = randomGrabAndDuckPlayer.unicornKey)
